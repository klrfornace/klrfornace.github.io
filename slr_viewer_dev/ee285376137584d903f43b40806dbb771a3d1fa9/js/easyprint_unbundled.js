    ...
    L.Control.EasyPrint = L.Control.extend({
        ...
        onAdd: function() {
            ...
            if (!this.options.hidden) {
                ...
                // for keyboard accessibility -KF
                this.link.href = '#';
                this.link.setAttribute('aria-label','Export map'),
                ...
                // Original code for multiple size modes
                    ...
                    this.options.sizeModes.forEach(function(t) {
                        ...
                        e.title = t.name;
                        var sizeLink = L.DomUtil.create("a", t.className, e);
                        sizeLink.href = '#';
        ...
        _togglePageSizeButtons: function(t) {
            if (this.holder){ // added to deal with case of simple button with no holder - KF
        ...
        _hideClasses: function(t) {
            if (!this.options.hideClasses)
                return;
            this.options.hideClasses.forEach(function(c) {
                var e = document.getElementsByClassName(c)[0];
                if (!e)
                    return;
                if (t)
        ...
        _showClasses: function(t) {
            if (!this.options.showClasses)
                return;
            this.options.showClasses.forEach(function(c) {
                var e = document.getElementsByClassName(c)[0];
                if (!e)
                    return;
                if (t)
